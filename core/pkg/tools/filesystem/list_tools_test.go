...
func (s *FilesystemTestSuite) TestListDirectoryWithSizes() {
	...
	// Sort by name
	args, _ := json.Marshal(map[string]any{"path": s.testDir, "sortBy": "name"})
	req := &mcp.CallToolRequest{
		Params: &mcp.CallToolParamsRaw{
	...
	res, err := s.fsServer.handleListDirectoryWithSizes(s.ctx, req)
	s.Require().NoError(err)
	s.Require().False(res.IsError)
	...
	// Sort by size
	args, _ = json.Marshal(map[string]any{"path": s.testDir, "sortBy": "size"})
	req.Params.Arguments = args
	res, err = s.fsServer.handleListDirectoryWithSizes(s.ctx, req)
	s.Require().NoError(err)
	s.Require().False(res.IsError)
	...
	for i := range output {
		if output[i:min(i+len(lines[0]), len(output))] == lines[0] { largeIdx = i }
		if output[i:min(i+len(lines[1]), len(output))] == lines[1] { smallIdx = i }
	}
...
func (s *FilesystemTestSuite) TestDirectoryTree() {
	...
	for _, node := range tree {
		if node.Name == "src" { foundSrc = true }
		if node.Name == "docs" { foundDocs = true }
		if node.Name == "vendor" { foundVendor = true }
	}
	s.True(foundSrc)
	s.True(foundDocs)
	s.False(foundVendor)
